...
export const heroStyles = {
  ...
  logoBlock: {
    ...
    wrapper: [
      ...
      'gap-2', // Расстояние между логотипом и заголовком
  ...
  background: {
    ...
    wrapper: [
      ...
      'overflow-hidden' // Скрыть переполнение, чтобы видео не выходило за пределы
    ].join(' '),
    ...
    video: [
      ...
      'object-cover', // Масштабирование видео для покрытия всей области
      'z-0' // Расположение под основным контентом
    ].join(' '),

    // Затемнение фона (необходимо для улучшения читаемости текста поверх видео)
    overlay: [
      'absolute', // Абсолютное позиционирование
      'inset-0', // Растягивание на всю секцию
      'bg-black', // Черный цвет
      'bg-opacity-50' // Прозрачность 50%
    ].join(' '),
